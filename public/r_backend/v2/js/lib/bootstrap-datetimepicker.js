...
!function ($) {
	...
	Datetimepicker.prototype = {
		...
		fill: function () {
		    ...
		    var addSec = /,s{1,2},/.test(',' + this.format.parts.join(',') + ','), sSec = addSec ? '秒：<select class="second-wrapper">' : '';
			this.addSec = addSec;
			if (addSec) {
			...
			var _this=this;
			$('.second-wrapper').change(function(){
				var year = _this.viewDate.getUTCFullYear(),
				month = _this.viewDate.getUTCMonth(),
				day = _this.viewDate.getUTCDate(),
				hours = _this.viewDate.getUTCHours(),
				minutes = _this.viewDate.getUTCMinutes(),
				seconds = _this.addSec ? _this.picker.find('select').val() : _this.viewDate.getUTCSeconds();
				_this._setDate(UTCDate(year, month, day, hours, minutes, seconds, 0));
				$('.datetimepicker').hide();
