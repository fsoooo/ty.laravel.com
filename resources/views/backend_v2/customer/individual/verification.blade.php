...
@section('main')
    ...
    @include('backend_v2.customer.individual.nav', ['type' => 'verification'])
        ...
        <div class="row autonym">
            @if(empty($user->card_img_front) || empty($user->card_img_backend) || empty($user->card_img_person) || empty($user->code))
            <div class="col-lg-12">
                <div class="certificate">
                        <div class="img-wrapper col-lg-3">
                            @if(empty($user->code))
                                没有证件号码
                            @endif
                            @if(empty($user->card_img_front) || empty($user->card_img_backend) || empty($user->card_img_person))
                                没有上传全部证件文件
                            @endif
                        </div>
                </div>
            ...
            <div class="col-lg-12">
                <div class="certificate">
                        ...
                        <div class="img-wrapper col-lg-3">
                            <img src="{{ rtrim(env('APP_URL'), '/'). '/' . $user->card_img_person }}" alt="" /><span class="btn btn-default" data-toggle="modal" data-target="#larger-3">查看大图</span>
                        </div>
                </div>
            </div>
            <div class="operation">
    ...
    <script>
        $(function () {
            $('#pass-button').on('click', function () {
                var user_id = '{{ $user->id }}';
                $.ajax({
                    url: '{{ route('backend.customer.individual.verification.store') }}',
                    data: {
                        user_id: user_id,
                        type: 'pass'
                    },
                    ...
                    success: function (result) {
                        if (result.code == 0) {
                            Mask.alert('审核通过');
                            location.href = '{{ route('backend.customer.individual.detail', [$user->id]) }}';
                        }
                    },
