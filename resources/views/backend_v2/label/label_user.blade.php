...
@section('main')
    <?php
    $user_active =  preg_match("/user_type=user/",Request::getRequestUri()) ? "active" : '';
    $company_active = preg_match("/user_type=company/",Request::getRequestUri()) ? "active" : '';
    ?>
		<div class="main-wrapper">
			<div class="row">
				<div class="section">
					 <div class="col-md-6 col-xs-6">
						 <a href="/backend/label/user_label?user_type=user" class="section-item @if($user_active==""&&$company_active=="") active @endif {{$user_active}}">
					 		<h4 class="title">个人用户</h4>
					 	</a>
				    </div>
				    <div class="col-md-6 col-xs-6">
				    	<a href="/backend/label/user_label?user_type=company" class="section-item {{$company_active}}">
				    		<h4 class="title">公司用户</h4>
				    	</a>
				    </div>
				</div>
			</div>
			...
			<form action="/backend/label/do_add_label" method="post" id="add_label">
				{{ csrf_field() }}
				<input type="hidden" name="label_belong" value="user">
				...
				@if(isset($_GET['user_type']))
					<input type="hidden" name="label_type" value="{{$_GET['user_type']}}">
				@else
					<input type="hidden" name="label_type" value="user">
				@endif
				<div class="row">
					<div class="col-lg-12">
	...
	<script>
        var user_type = "@if(isset($_GET['user_type'])){{$_GET['user_type']}}@endif";
        var url = "{{Request::getPathinfo()}}";
        var params = /\?/;
        var result = params.test(url);
        function get_label(id) {
            if(user_type==''||user_type==null||typeof(user_type)==undefined){
                user_type = 'user';
			}
            if(result){
                window.location.href = url+"?user_type="+user_type+"&"+"search_type=label_group&keyword="+id;
            }else{
                window.location.href = url+"?user_type="+user_type+"&search_type=label_group&keyword="+id;
            }
        }
