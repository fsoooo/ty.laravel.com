...
@section('main')
							...
							<div class="policy-list">
								...
								<p><span class="name">条款</span><i>:</i>
									@foreach($item['clauses'] as $v)
										<li></li><a href="{{env('TY_API_PRODUCT_SERVICE_URL').'/'.$v['file_url']}}">{{$v['name']}}</a></li>
									@endforeach
								</p>
								...
								<p><span class="name">保 费</span><i>:</i>{{ceil($product->premium/100)}}元</p>
								...
								<p><span class="name">代理人</span><i>:</i>{{isset($agent_name->real_name) ? "$agent_name->real_name" : "--" }}</p>
		...
		<script>
			...
			$(".clause-list").panel({iWheelStep:32});
