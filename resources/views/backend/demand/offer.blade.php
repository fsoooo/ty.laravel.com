                        ...
                        <div class="main-box" style="min-height: 1100px;">
                            ...
                            <div class="main-box-body">
                                <div class="table-responsive">
                                    ...
                                    <tr>
                                        <td><a href=""></a><button class="md-trigger btn btn-primary mrg-b-lg pull-right" data-modal="modal-8" id="add">添加产品</button></td>
                    ...
                    <div class="form-group">
                        ...
                        <select name="product_id" id="product-id" class="form-control">
                            ...
                            @foreach($product_list as $value)
                                <option value="{{ $value->id }}">{{ $value->product_name }}-{{$value->company_name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
@section('foot-js')
    ...
    <script>
        $(function(){console.log('sdf');
            ...
            {
                var product_id_val = $('#product-id option:selected').val();
                var product_name_val = $('#product-id option:selected').html();
				var company =$.trim($('#product-display').children('p').text()).split("-")[1];
				var company_selected = product_name_val.split("-")[1];
//				alert(company_selected);
				if (typeof(company) != 'undefined') {
					if (company != company_selected){
						alert('请选择同一家公司的产品');
						return false;
					}
				}
