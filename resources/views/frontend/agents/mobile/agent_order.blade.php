...
<head>
    ...
    <style>
        .product-wrapper .name{text-decoration: underline;}
        .status{ position: absolute;right: 0.4rem;bottom: -.5rem;font-size: .28rem;color: #ffae00;}
        .zbtn-default{color: #00a2ff;border: 1px solid #00a2ff;background: none;}
    </style>
</head>
                            ...
                            <ul>
                                ...
                                <li>
                                    ...
                                    <div class="product-right">
                                        <p>{{$v->order['premium']/100}}元</p>
                                        <a href="/agent_sale/plan_detail_other/{{$v->id}}" class="zbtn zbtn-default">查看详情</a>
                                        <span class="status">
                                            @if($v['status'] == 0)
                                                未发送
                                            @else
                                    ...
                                    <h3 class="name" style="width:12em;display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><a href="/agent_sale/plan_detail/{{$v->id}}">{{$v->name}}</a></h3>
                                    <p class="num" style="margin: .4rem 0 0;"><span class="color-positive">推广费:{{$v->rate['earning']}}%</span></p>
                                    @if($v->json['type'] == 2)
                                        <p class="info" style="margin: 0;"><span>投保人：{{$v->order->order_user['real_name']}}</span><span>被保人：{{$v->order->warranty_recognizee[0]['name']}}</span></p>
                                        @elseif($v->json['type'] == 1)
                                        <p class="info" style="margin: 0;"><span>投保企业：{{$v->order->order_user['real_name']}}</span></p>
                                        <i class="icon-tuan"></i>
                                        @else
                                        --
                                    @endif
...
<script>
    ...
    $('#search').bind('search', function() {
        ...
        $("input").blur();
        location.href='/agent_sale/agent_order'+'?search='+search_data;
    });
</script>
</body>
